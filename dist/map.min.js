...
var InteractionPage = function () {
    function InteractionPage(map, config, popup) {
        ...
        this.viewportMetaTag = [].slice.call(document.getElementsByName("viewport"), 0)[0];
        this.htmlDoc = document.getElementsByTagName('html')[0];
        this.filter = ['has', 'slug'];
    ...
    InteractionPage.prototype.hoversize = function hoversize() {
        ...
        if (!mask || mask === null || mask === undefined) {
            ...
            // voeg mask in VOOR de sluit knop.
            self.hostContainer.insertBefore(mask, self.closeButton);
        }
    ...
    InteractionPage.prototype.fullsize = function fullsize() {
        ...
        if (isIos()) {
            ...
            // let inlineMapContainer = self.hostContainer.cloneNode(true);
            // self.hostContainer.remove();
            self.initialScrollPosition = self.hostContainer.scrollTop;
            // self.hostContainer.classList.add('inline');
            self.hostContainer.style.position = 'relative';
            self.hostContainer.style.overflow = 'auto';
            self.hostContainer.style.top = 'auto';
            self.hostContainer.style.left = 'auto';
            self.hostContainer.style.right = 'auto';
            self.hostContainer.style.bottom = 'auto';
            self.hostContainer.style.height = 'calc(100vh - 4rem)'; // 3.5
            self.body.appendChild(self.hostContainer);
            window.scrollTo(0, document.body.scrollHeight);
        } else {

            self.htmlDoc.classList.add('allabouthemap');
            self.hostContainer.style.zIndex = '9999';
        }

        if (webgl_detect() && !isIE11()) {

            self._map.setLayoutProperty('icons-active', 'icon-size', 1.5);
            // hier moet een shift pitch bij
            self.interaction3D = new Interaction3D(self._map, self._config);
            self.interaction3D.init(false);
            self.interaction3D.shiftPitch(self._map, 75);

            // self.setActiveMarker(self._config.activeItems);

        } else {

                // console.log(self._config.activeItems);
                // console.log(self._config.dataset.content);
                //
                // self._config.dataset.content.features.forEach(function(item) {
                //
                //     if (self._config.activeItems.indexOf(item.properties.slug) > -1) {
                //
                //         html = self.interactionPopup.createPopup(item);
                //         latLng = new L.LatLng(item.geometry.coordinates[1],item.geometry.coordinates[0]);
                //         //open popup;
                //         popup = L.popup({
                //             className: item.properties.type,
                //             offset: [0, 36]
                ...
                //             .setContent(html)
                //             .openOn(self._map);
                //         self.interactionPopup.addToArray(popup)
                //     }
                // });
            }
    };
    ...
    InteractionPage.prototype.close = function close() {
        ...
        if (isIos()) {
            window.scrollTo(0, self.initialScrollPosition);
            self.hostContainer.style.position = 'fixed';
            self.hostContainer.style.overflow = 'hidden';
            self.hostContainer.style.top = '13.5rem';
            self.hostContainer.style.right = '0';
            if (window.innerWidth > 760) {
                self.hostContainer.style.height = '17rem';
            } else {
                self.hostContainer.style.height = '4.5rem';
            }
        } else {
            self.htmlDoc.classList.remove('allabouthemap');
        }

        self.interactionPopup.emptyArray(self._map);

        if (webgl_detect() && !isIE11()) {
            // self._map.setPaintProperty('icons', 'icon-opacity', .4);
            self.interaction3D.resetPitch(self._map);
            self._map.setLayoutProperty('icons-active', 'icon-size', 1);
        }
    };
...
var Map = function () {
    function Map(element, data, interaction) {
        ...
        this.config = {
            ...
            hostContainer: element, // !== undefined ? document.getElementById(element) : document.getElementsByClassName('datavis-map')[0],
            center: [4.875, 52.3],
            zoom: 5,
            pitch: 0,
            bearing: 0
            // activeItems: activeItems,
            // scrollzoom: (typeof self.customConfig.scrollzoom === 'undefined') ? false : self.customConfig.scrollzoom,
            // highlights: (typeof self.customConfig.highlights === 'undefined') ? true : self.customConfig.highlights
    ...
    Map.prototype.init = function init() {
        ...
        if (webgl_detect() && !isIE11()) {
            ...
            // if (self.customConfig && self.customConfig.interaction === 'navigation') {
            ...
            // }
            //
            // if (self.customConfig && self.customConfig.filter) {
            //     self.interactionFilters = new InteractionFilters(self._map, self.config,self._content,self.interactionPopup);
            ...
            self._map.on('style.load', function () {
                ...
                // self._donut.draw();
                //
                // if(self.customConfig && self.customConfig.poi === true) {
                //     self._poi.draw();
                // }
                //
                ...
                // if(self.config.dataset.lines && self.config.dataset.lines.features.length > 0) {
                //     self._lines.draw(self.config.dataset.lines);
                // }
                //
                ...
                // self.interaction3D = new Interaction3D(self._map, self.config);
                // if (self.customConfig && self.customConfig.pitch) {
                //     self.interaction3D.init(true);
                // } else {
                //     self.interaction3D.init(false);
                // }
            });
        ...
        // if (self.options.referenceList) {
        //     self.listReference =  new ListReference(self.interactionPopup);
        //
        //     if(self.config.dataset.content) {
        //         let html = self.listReference.generateHTML(self.config.dataset.content.features);
        //         let listContainer = document.getElementById('map-list-container');
        //         if (listContainer) {
        //             listContainer.classList.add('visible');
        //             listContainer.appendChild(html);
        //             self.listReference.addEventListeners(self._map,listContainer);
        //         }
        //
        //         if (window.innerWidth < 761) {
        //
        //             let tabs = self.listReference.generateTabs();
        //             let tabContainer = document.getElementById('map-tabs-container');
        //
        //             if (tabContainer) {
        //                 tabContainer.appendChild(tabs);
        //                 self.listReference.addEventListenersMobile();
        //             }
        //         }
        //     }
        // }

        // open map on pageload
        // if (self.customConfig && self.customConfig.interaction === 'navigation' && window.innerWidth > 760) {
        //
        ...
        //         self.interactionPage.hoversize();
        //     }
        // }
    };
